#!/usr/bin/env python3
"""Beacon Spool legacy implementation."""
...
def process_inbox_job(path: Path, paths: dict[str, Path], crash_at: str | None) -> None:
...
def run(root_argument: str, crash_at: str | None) -> int:
    ...
    for path in candidate_files(paths["inbox"]):
        process_inbox_job(path, paths, crash_at)
    return 0
